...
@RestController
@RequestMapping(value = "/componentDataService/orderManagmentCsrController")
public class OrderManagmentCsrController {

	@Autowired
	OrderManagmentService orderManagment;
	...
	public ResponseEntity<?> placeOrder(@RequestBody Map<String, Object> request) {
		try {
			return new ResponseEntity<>(orderManagment.placeOrder(request), HttpStatus.OK);
	...
	@RequestMapping(method = { RequestMethod.POST }, value = { "/serchOrderByCriteria" }, produces = {
			"application/json" })
	public ResponseEntity<?> serchOrderByCriteria(@RequestBody Map<String, Object> filter) {
		try {
			return new ResponseEntity<>(orderManagment.getOrdersByFilter(filter), HttpStatus.OK);
		} catch (HttpClientErrorException ex) {
			return new ResponseEntity<>(DataComponent.errorResponse(ex.getResponseBodyAsString()), HttpStatus.OK);
		}
